<template>
  <div class="app-container">
    <div class="title">{{$t('statistics.ContractApprovalCompletionRateStatistics')}}</div>
    ...
    <el-table v-loading="loading" :data="dataList" style="margin-top:20px"
      :span-method="objectSpanMethod" border >
        ...
        <el-table-column :label="$t('statistics.dividend')" align="center" prop="dividend" ></el-table-column>
        <el-table-column :label="$t('statistics.divisor')" align="center" prop="divisor" ></el-table-column>
        <el-table-column :label="$t('statistics.rate')" align="center" prop="rate" ></el-table-column>
    </el-table>
  </div>
</template>

<script>
import {contractApprovalCompletionRate} from "@/api/system/statistics";
...
export default {
  ...
  methods: {
    getList(){
      this.loading = true
      contractApprovalCompletionRate().then((response) => {
