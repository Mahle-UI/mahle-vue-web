  ...
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        ...
        >{{$t('flowAuth.batchSetting')}}</el-button>
        <el-button
          type="warning"
          size="mini"
          @click="handleBatchCancelSetting"
          v-hasPermi="['flow:contract:export']"
        >{{$t('flowAuth.batchCancelSetting')}}</el-button>
      </el-col>
      ...
      <el-table-column :label="$t('common.operation')" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            ...
            type="text"
            icon="el-icon-edit"
            @click="handleSetting(scope.row)">{{$t('common.setting')}}</el-button>
          <el-button
            v-else
            size="mini"
            type="text"
            icon="el-icon-edit" @click="handleCancel(scope.row)">{{$t('common.cancle')}}{{$t('common.setting')}}</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :close-on-click-modal="false" :title="title" :visible.sync="open" width="500px" append-to-body>
          <el-form ref="form" :model="form" :rules="rules" :label-width="$i18n.locale=='en'?'150px':'80px'">
            ...
            <el-form-item :label="$t('flowAuth.authUser')" prop="agentId"> <el-select v-model="form.agentId" style="width: 100%"
